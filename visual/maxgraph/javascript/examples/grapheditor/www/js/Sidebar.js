...
Sidebar.prototype.addBasicPalette = function(dir)
{
	...
	this.addStencilPalette('basic', mxResources.get('basic'), dir + '/basic.xml',';verticalLabelPosition=bottom;verticalAlign=top;whiteSpace=wrap;html=1;strokeColor=#ffffff;strokeWidth=2;fillColor=#3333FF;strokeWidth=2',null, null, null, null, [
	]);
	this.setCurrentSearchEntryLibrary();
};
		...
		mxStencilRegistry.loadStencilSet(stencilFile, mxUtils.bind(this, function(packageName, stencilName, displayName, w, h)
		{
			if (ignore == null || mxUtils.indexOf(ignore, stencilName) < 0)
			{
				...
				fns.push(this.createVertexTemplateEntry('shape=' + packageName + stencilName.toLowerCase() + style,
					Math.round(w * scale), Math.round(h * scale), stencilName.replace(/_/g, ' '), stencilName.replace(/_/g, ' '), null, null,
			...
			mxStencilRegistry.loadStencilSet(stencilFile, mxUtils.bind(this, function(packageName, stencilName, displayName, w, h)
			{
				if (ignore == null || mxUtils.indexOf(ignore, stencilName) < 0)
				{
					content.appendChild(this.createVertexTemplate('shape=' + packageName + stencilName.toLowerCase() + style,
						Math.round(w * scale), Math.round(h * scale), stencilName.replace(/_/g, ' '), stencilName.replace(/_/g, ' '), true));
				}
			}), true);
	    }));
	}
